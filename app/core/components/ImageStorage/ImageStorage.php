...
class ImageStorage
{
     ...
     * @return void
     */
    public function deleteCache($arg, $thumb = false)
    {
        if (is_object($arg) && $arg instanceof Image) {
            $script = ImageNameScript::fromIdentifier($arg->identifier);
        } else {
        ...
        foreach (new \DirectoryIterator($dir) as $file_info) {
            if (preg_match($pattern, $file_info->getFilename())) {
                if (!$thumb && $script->name.'.'.$script->extension != $file_info->getFilename() && $file_info->getExtension() == $script->extension && stripos($file_info->getFilename(), 'thumb') === false) {
                    unlink($file_info->getPathname());
                } elseif ($thumb && $script->name.'.'.$script->extension != $file_info->getFilename() && $file_info->getExtension() == $script->extension && stripos($file_info->getFilename(), 'thumb') !== false) {
                    unlink($file_info->getPathname());
                }
            }
        }
	...
	public function fromIdentifier($args)
	{
		...
		if (!file_exists(implode('/', [$this->data_path, $identifier]))) {
            ...
            if ($watermark) {
                ...
                $wm->resize($_image->getWidth() / 100 * 33.33, null);
                $_image->place($wm, '0%', '100%', 25);
            }
