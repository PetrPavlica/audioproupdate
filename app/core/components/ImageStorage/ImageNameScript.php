...
class ImageNameScript
{
	...
	public static function fromName($name)
	{
        ...
        if (count($matches) > 15) {
            ...
            $script->original = $matches[1] . '/' . $matches[2] . '.' . $matches[14];
            $script->namespace = $matches[1];
            $script->prefix = null;
            $script->name = $matches[2];
            $script->size = [(int)$matches[4], (int)$matches[5]];
            $script->flag = $matches[11];
            $script->quality = $matches[13];
            $script->extension = $matches[14];

            if ($matches[7] && $matches[8] && $matches[9] && $matches[10]) {
                $script->crop = [(int)$matches[7], (int)$matches[8], (int)$matches[9], (int)$matches[10]];
            }
        }
	...
	{
		$identifier = implode('/', array_filter([$this->namespace, $this->prefix, $this->name]));
