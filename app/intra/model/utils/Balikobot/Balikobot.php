...
namespace Intra\Model\Utils\Balikobot;
...
class Balikobot
{
    ...
    const
        ...
        OPTION_WIDTH = 'width',
        OPTION_ORDER_NUMBER = 'order_number',
        OPTION_EID = 'eid',
        OPTION_RETURN_TRACK = 'return_track',
        OPTION_CREDIT_CARD = 'credit_card';
    ...
    const
        ...
        EXCEPTION_INVALID_COMBINED_REQUEST = 403,
     ...
     * @return $this
     */
    public function service($keyPackage, $shipper, $service, array $options = [])
    {
        ...
        foreach ($options as $name => $value) {
            $this->saveOption($keyPackage, $name, $value, $shipper);
        }
        $this->data['packages'][$keyPackage]['service_type'] = $service;
        if (!isset($this->data['packages'][$keyPackage]['eid']) || !$this->data['packages'][$keyPackage]['eid']) {
            $orderId = isset($this->data['packages'][$keyPackage][self::OPTION_ORDER]) ? sprintf('%\'010s',
                $this->data['packages'][$keyPackage][self::OPTION_ORDER]) : '0000000000';
            if ($keyPackage == 0) {
                $this->data['packages'][$keyPackage]['eid'] = $this->getEid(null, $orderId);
            } else {
                $this->data['packages'][$keyPackage]['eid'] = $this->data['packages'][0]['eid'];
            }
        }
        $this->data['packages'][$keyPackage]['return_full_errors'] = true;
    ...
    public function customer(
        ...
        $this->data['packages'][$keyPackage]['rec_name'] = $name;
        $this->data['packages'][$keyPackage]['rec_street'] = $street;
        $this->data['packages'][$keyPackage]['rec_city'] = $city;
        $this->data['packages'][$keyPackage]['rec_zip'] = $zip;
        $this->data['packages'][$keyPackage]['rec_phone'] = $phone;
        $this->data['packages'][$keyPackage]['rec_email'] = $email;
        $this->data['packages'][$keyPackage]['rec_country'] = $country;
        if (isset($company)) {
            $this->data['packages'][$keyPackage]['rec_firm'] = $company;
        }
     ...
     * @return $this
     */
    public function cashOnDelivery($keyPackage, $price, $variableSymbol, $currency = self::CURRENCY_CZK)
    {
        ...
        $this->data['packages'][$keyPackage]['cod_price'] = (float)$price;
        $this->data['packages'][$keyPackage]['vs'] = $variableSymbol;
        $this->data['packages'][$keyPackage]['cod_currency'] = $currency;
    ...
    public function add()
    {
        ...
        $response = $this->call(self::REQUEST_ADD, $this->data['shipper'], $this->data['packages']);
        $this->clean();

        if (!isset($response[0]['package_id'])) {
            throw new \InvalidArgumentException('Invalid arguments. Errors: ' . serialize($response[0]) . '.',
    ...
    public function getServices($shipper)
    {
        ...
        if (isset($response['status']) && ($response['status'] == 409)) {
            throw new \InvalidArgumentException("The $shipper shipper is not supported.",
                self::EXCEPTION_NOT_SUPPORTED);
        }
        if (!isset($response['status']) || ($response['status'] != 200)) {
            $code = isset($response['status']) ? $response['status'] : 0;
            throw new \UnexpectedValueException("Unexpected server response, code = $code.",
    ...
    public function dropPackage($shipper, $packageId)
    {
        ...
        if ($response['status'] == 403) {
            throw new \UnexpectedValueException('The package was not added as the last one.',
                self::EXCEPTION_INVALID_COMBINED_REQUEST);
        }
        elseif ($response['status'] == 404) {
            throw new \UnexpectedValueException('The package does not exist or it was ordered.',
                self::EXCEPTION_INVALID_REQUEST);
        }
        if ($response['status'] != 200) {
            throw new \UnexpectedValueException("Unexpected server response, code={$response['status']}.",
                self::EXCEPTION_SERVER_ERROR);
        }
    }
    ...
    public function getLabels($shipper, array $packages)
    {
        ...
        if (isset($response['status']) && ($response['status'] != 200)) {
            throw new \UnexpectedValueException('Invalid data or invalid packages number.',
                self::EXCEPTION_INVALID_REQUEST);
        }
     ...
     * @param mixed $value
     */
    private function saveOption($keyPackage, $name, $value, $shipper = null)
    {
        if (empty($name)) {
            throw new \InvalidArgumentException('Invalid argument has been entered.');
        }
        ...
        $this->data['packages'][$keyPackage][$name] = $value;
    }
